    ...
    def _gender_marker_significance(self):
        ...
        for policy_marker in policy_markers:
            if 'vocabulary' not in policy_marker or policy_marker.attrib['vocabulary'] == '1':
                return SimpleField(policy_marker.attrib.get('significance'))
        return SimpleField(None)

    def _activity_start_date(self):
        actual_start_date = self.activity.find('activity-date[@type="2"]')
        planned_start_date = self.activity.find('activity-date[@type="1"]')
        if actual_start_date is not None:
            return SimpleField(actual_start_date.attrib.get('iso-date'))
        elif planned_start_date is not None:
            return SimpleField(planned_start_date.attrib.get('iso-date'))
        else:
            return SimpleField(None)
...
class ActivityBudget(Common):
    ...
    def generate(self):
        ...
        self.activity_start_date = self._activity_start_date()
        return True
